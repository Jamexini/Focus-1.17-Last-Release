...
async function init() {
    ...
    if (!state.allShieldSites || state.allShieldSites.length === 0) {
        ...
        // Ensure active items are in inventory
        if (state.shieldList) {
    ...
    updateProgressRing();
    renderSiteList();
    userUpdateShieldUI(); // Load shield UI state
    loadShieldSettings(); // Load hotkey & mode
    updateStats();
    loadSettings();
    ...
    if (state.lastActiveTab) {
        ...
        const activeBtn = document.querySelector(`.tab-btn[data-tab="${state.lastActiveTab}"]`);
        const activeContent = document.getElementById(`tab-${state.lastActiveTab}`);
...
function renderShieldList() {
    ...
    state.allShieldSites.forEach(site => {
        ...
        item.innerHTML = `
            ...
            <button class="small-btn" style="background:transparent; color:var(--text-dim); padding:4px;" title="Remove">
                <svg viewBox="0 0 24 24" width="16" height="16" stroke="currentColor" stroke-width="2" fill="none" stroke-linecap="round" stroke-linejoin="round"><line x1="18" y1="6" x2="6" y2="18"></line><line x1="6" y1="6" x2="18" y2="18"></line></svg>
            </button>
        `;
        item.querySelector('input[type="checkbox"]').onchange = (e) => toggleShieldSite(site.domain, e.target.checked);
        item.querySelector('button').onclick = () => removeShieldSite(site.domain);
...
function removeShieldSite(domain) {
    ...
    state.shieldList = state.shieldList.filter(d => d !== domain); // Also remove from active list if present
...
el.shieldHotkeyInput.addEventListener('keydown', (e) => {
    e.preventDefault();
    e.stopPropagation();
    // Only accept combos with at least one modifier + a letter
